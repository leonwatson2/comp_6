...
public class SymbolTable {
  ...
  //Adds code to the itermidiate code of env.
  ...
  public void printFunction(SymbolTableEntry funcOrClass, String funcName){
    ...
    while (envIterator . hasNext ()) {
      ...
      String id = entry . getKey ();
      SymbolTableEntry idEntry = entry . getValue ();
      Type type = idEntry.type();
      ArrayType aType;
     if(type.getClass() == ArrayType.class){
            aType = (ArrayType) type;
            System.out.println(aType.baseType() + " " + id + "["+aType.dimension()+"]");
     }else{
    ...
    while(i < numberOfTemps){
      System.out.println("int _T" + (i++) + ";");
    }
    System.out.println(intermediateCode);

      // if (idEntry . category () == Category . CLASS 
      //     || idEntry . category () == Category . FUNCTION){
      //  idEntry . localEnv () . printFunction (id); 
